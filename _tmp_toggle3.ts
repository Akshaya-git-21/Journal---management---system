...
async function main() {
  const { error } = await supabase.from("manuscripts").update({ reviews_released_at: null }).eq("id", "JMS-2026-DDKPE");
  console.log(error ? error.message : "cleared reviews_released_at");
}
