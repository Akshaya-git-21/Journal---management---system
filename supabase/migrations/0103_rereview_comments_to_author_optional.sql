-- ==========================================
-- Module 103: a re-review round's Comments to Author is optional unless the
-- reviewer is rejecting the revision (the frontend already reflects this --
-- see ReviewerWorkspace.tsx's validateForm) -- submit_peer_review() must stop
-- unconditionally requiring it, or every re-review submission with an empty
-- comment fails server-side with "Comments to Author are required" even
-- though the UI no longer marks the field mandatory.
--
-- Depends on: 0044_reviewer_rereview_skip_questionnaire.sql.
-- Safe to re-run.
-- ==========================================
...
begin
  ...
  -- First round: Comments to Author is always required. Re-review round:
  -- only required when rejecting the revision (the reason for the reject).
  if (not is_rereview_round or p_recommendation = 'REJECT') and coalesce(trim(p_comments_to_author), '') = '' then
