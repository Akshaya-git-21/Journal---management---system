 ...
 * reverse-chronological list. Shared across the Coordinator's oversight
 * views and the Editor's/Author's own compact round history. */
export default function ProofReviewTimeline({ manuscriptId, variant = 'full' }: { manuscriptId: string; variant?: 'full' | 'compact' }) {
  ...
  if (entries.length === 0) {
    return <p className="text-sm text-slate-400">No proof activity yet.</p>;
  }
    ...
    <div className={variant === 'compact' ? 'space-y-2' : 'space-y-3'}>
      ...
      {entries.map((entry, i) => {
        if (entry.kind === 'upload') {
          const p = entry.proof;
          return (
            <div key={`upload-${p.id}`} className="rounded-xl border border-slate-200 bg-white p-3 text-xs space-y-1">
        ...
        return (
          <div key={`review-${r.id ?? i}`} className={`rounded-xl border p-3 text-xs space-y-1 ${r.superseded_at ? 'border-slate-100 bg-slate-50 opacity-70' : 'border-slate-200 bg-white'}`}>
