...
import { getReviewerDisplayStatus, ReviewerDisplayStatus } from '../../../lib/reviewerStatus';
...
export function ReviewersTab({ reviewerAssignments, profiles }: Props) {
  const getStatusIcon = (status: ReviewerDisplayStatus) => {
    switch (status) {
      case 'SUBMITTED':
  ...
  const getStatusColor = (status: ReviewerDisplayStatus) => {
    switch (status) {
      case 'SUBMITTED':
      ...
      {reviewerAssignments.length === 0 ? (
        ...
        <div className="space-y-4">
          {reviewerAssignments.map((reviewer, idx) => {
            ...
            const displayStatus = getReviewerDisplayStatus(reviewer);
            return (
              <div key={reviewer.id} className="bg-white border border-slate-200 rounded-2xl p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`flex items-center gap-1 px-3 py-1 text-xs font-bold rounded-full ${getStatusColor(displayStatus)}`}>
