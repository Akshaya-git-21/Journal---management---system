...
export default function CoordinatorWorkspace(_props: CoordinatorWorkspaceProps) {
  ...
  const handleSendInvite = async () => {
    ...
    const email = normalizedEmail || `${normalizedName.replace(/\s+/g, '.').toLowerCase()}@example.com`;
    const password = invitePassword.trim() || generateTempPassword();

    try {
      setLoading(true);
      const result = await registerAccount(email, password, normalizedName, 'EDITOR', {
        specialization: inviteDiscipline.trim(),
      ...
      for (let attempt = 0; attempt < 8; attempt += 1) {
        const { data, error } = await supabase.from('profiles').select('id, email, status').eq('email', email).maybeSingle();
        if (!error && data) {
