/**
 * Vercel serverless function -- production entrypoint for the Admin "manage
 * users" operations. Self-contained on purpose (see api/reset-user-password.ts);
 * the core below is identical to src/lib/adminUsersHandler.ts -- change both
 * together.
 */
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await runAdminUsersRequest(admin, req.headers?.authorization, body);
    res.status(result.status).json(result.body);
  } catch (error: any) {
    console.error('[api/admin-users] Unexpected error:', error);
    res.status(500).json({ error: error?.message || 'Unable to manage users.' });
  }
}
